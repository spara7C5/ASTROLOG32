...
void WriteXBitmap(FILE *file, char *name, byte mode) {
	int x, y, i, temp = 0;
	uint value;
	...
	for (y = 0; y < gs.yWin; y++)
	{
		...
		do
		{
			...
			if ((mode == 'N' && temp >= 12) || (mode == 'C' && temp >= 15) || (mode == 'V' && temp >= 11))
				temp = 0;
			x += (mode != 'V' ? 8 : 16);
		}
		while (x < gs.xWin);
...
void WriteBmp(FILE *file) {
	...
	if (gi.yBand == 0 || gi.yOffset + gi.yBand >= gs.yWin)
	{
		...
		if (gs.fColor)
			for (x = 0; x < 16; x++)
			{
				PutByte(RGBB(rgbbmp[x]));
				PutByte(RGBG(rgbbmp[x]));
				PutByte(RGBR(rgbbmp[x]));
				PutByte(0);
			}
		else
		{
...
// Begin the work of creating a graphics file. Create the file and open it for writing
...
void BeginFileX()
{
	...
	{
		PrintWarning("Undefined output file.");
		return;
	}
...
void EndFileX()
{
	...
	if (wi.wCmd == cmdSaveWallTile || wi.wCmd == cmdSaveWallCenter ||
			wi.wCmd == cmdSaveWallStretch)
	{
		LONG	result;
		HKEY	desktop_key;

		result = RegOpenKeyEx(HKEY_CURRENT_USER, "Control Panel\\Desktop", 
			0, KEY_SET_VALUE, &desktop_key);

		if(result)
		{
			PrintWarning("RegOpenKeyEx failed with error %d", result);
		}
		else
		{
			result = RegSetValueEx(desktop_key, "TileWallpaper", 0, REG_SZ,
				(BYTE*) (wi.wCmd == cmdSaveWallTile ? "1" : "0"), 2);
 
			if(result)
			{
				PrintWarning("RegSetValueEx failed with error %d for TileWallpaper", result);
			}

			result = RegSetValueEx(desktop_key, "WallpaperStyle", 0, REG_SZ,
				(BYTE*) (wi.wCmd == cmdSaveWallStretch ? "2" : "0"), 2);
			...
			result = RegSetValueEx(desktop_key, "Wallpaper", 0, REG_SZ,
				(BYTE*)gi.szFileOut, strlen(gi.szFileOut) + 1);
 
			if(result)
			{
				PrintWarning("RegSetValueEx failed with error %d for Wallpaper", result);
			}
...
const char szPsFunctions[] = "/languagelevel where{pop languagelevel}{1}ifelse\
 2 lt{\n\
...
void PsLineWidth(int linewidth) {
	if ((double) linewidth != gi.rLineWid)
	{
		PsStrokeForce();
		fprintf(gi.file, "%d setlinewidth\n", linewidth);
		gi.rLineWid = (double) linewidth;
	}
}
...
void PsBegin()
{
	...
	else
	{
		...
		fprintf(gi.file, "%%%%BoundingBox: %d %d %d %d\n", PSGUTTER,
				PSGUTTER, (int) (gs.xInch * 72.0 + 0.5) - PSGUTTER, (int) (gs.yInch * 72.0 + 0.5) - PSGUTTER);
		fprintf(gi.file, "%%%%EndComments\n");
...
void MetaInit()
{
	...
	MetaWord(0);
	MetaWord(0);
	MetaWord((word) gs.xWin);
	MetaWord((word) gs.yWin);
	MetaWord((word) (gs.xWin / 6));	/* Units per inch */
	MetaLong(0L);				/* Not used */
	MetaWord((word) (0x9AC6 ^ 0xCDD7 ^ gs.xWin ^ gs.yWin ^ gs.xWin / 6));	/* Checksum */
	/* Metaheader */
	...
	MetaWord((word) (16 * 5 + 1 + (gs.fFont > 0) * 4));	/* Number of objects in metafile    */
...
void WriteMeta(FILE *file) {
	...
	*(long *) (gi.bm + 22 + 6) = ((long) ((pbyte) gi.pwMetaCur - gi.bm) - 22) / 2;
